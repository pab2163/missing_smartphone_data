...
```{r}
# coef_within_si_predict_missingness_weekly_for_plot = dplyr::filter(coef_within_si_predict_missingness_weekly,
#                                                                    term == 'si_within') %>%
...
#          plot_label = paste0('\u03B2', '=', round(estimate, 2), ', 95% CI [', round(conf.low, 2), ', ', round(conf.high, 2), ']'),
...
missingness_predict_si_plot = ggplot(missingness_predict_si_model_predictions, aes(x = missing_prop_lag, y = emmean)) +
  ...
  #stat_summary(data = raw_summary, fun.data = mean_cl_boot, aes(color = name), alpha = 0.8) +
  facet_grid(~name) +
  ...
  # geom_text(data =missingness_predict_si_coef_for_plot,
...
missingness_predict_si_plot_daily_gps = ggplot(missingness_predict_si_model_predictions %>% dplyr::filter(name %in% c('GPS', 'Daily Mood Survey'))
  ...
  theme(legend.position = 'none', panel.grid.minor = element_blank()) +
  # geom_text(data =missingness_predict_si_coef_for_plot %>% dplyr::filter(name %in% c('GPS', 'Daily Mood Survey')),
  #          aes(label = plot_label),
  #          x = 0, y = 0.3, hjust = 0) +
  scale_x_continuous(breaks = 0:7)
```
...
# models_si_binary_predict_missingness = daily_weekly_si %>%
#   pivot_longer(c(contains('missing'), -contains('lag1'))) %>%
#   group_by(name) %>%
#   nest() %>%
#   mutate(model = map(data, ~brms::brm(data = ., value ~ si_bin + summerbreak + personal_time_z + (si_bin|ID),
#                                         family = bernoulli(link = 'logit'), chains = 4, cores = 4, iter = 4000, seed = 11291993, prior = beta_prior)))
```
...
# models_si_binary_predict_missingness_weekly = daily_weekly_si_grouped_weeks %>%
#   pivot_longer(c(contains('prop_missing'))) %>%
#   group_by(name) %>%
#   nest() %>%
#   mutate(model = map(data, ~brms::brm(data = ., value ~ si_bin + summerbreak + personal_time_z + (si_bin|ID),
...
# coef_si_binary_predict_missingness_weekly = models_si_binary_predict_missingness_weekly %>%
#   mutate(coef = map(model, ~broom.mixed::tidy(., conf.int = TRUE))) %>%
#   unnest(coef) %>%
#   dplyr::select(-data, -model) 
...
si_models_curves_weekly_plot = models_si_predict_missingness_predictions_weekly %>%
  ...
  scale_color_manual(values = pal2) +
  theme(legend.position = 'none') +
  # geom_text(data =coef_within_si_predict_missingness_weekly_for_plot,
  #          aes(label = plot_label),
  #          x = 0.8, y = 6, hjust = 0) +
  ylim(0,7) 
...
si_models_curves_weekly_plot_gps_daily = models_si_predict_missingness_predictions_weekly %>%
  ...
  scale_color_manual(values = pal2[2:3]) +
  theme(legend.position = 'none') +
  # geom_text(data =coef_within_si_predict_missingness_weekly_for_plot %>% dplyr::filter(name %in% c('GPS', 'Daily Mood Survey')),
  #          aes(label = plot_label),
  #          x = 0.8, y = 6, hjust = 0) +
  ylim(0,7) 
...
# 3: Assemble grid for Marginal Predictions Plots
