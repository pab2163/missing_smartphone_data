...
or_plot_sobp = time_or_daily_basis %>%
  dplyr::filter(! term %in% c('(Intercept)', 'sd__(Intercept)')) %>%
  mutate(term = dplyr::recode(term,
                              'weekend'='Weekend > Week Days',
                              'summerbreak1'='Summer Break > School Year',
  ...
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0, position = position_dodge(0.2), lwd = 1) +
  geom_hline(yintercept = 1, lty = 2) +
  coord_flip() +
  scale_y_log10() +
  labs(y = 'Association with missingness\nAdjusted Odds Ratio',
...
timeplt_sobp = ggplot(daily_day_num_proportions, aes(x = week_num, y = p)) +
  geom_hline(yintercept = 0.5, lty = 2) +
  ...
  geom_label_repel(
    aes(label = round(p,2), color = name), data = time_starts,
    fontface ="plain",  size = 3, direction = 'both', point.padding = 1, show.legend =FALSE) +
  geom_label_repel(
    aes(label = round(p,2), color = name), data = time_ends,
    fontface ="plain",  size = 3, direction = 'both', show.legend =FALSE) +
  scale_color_manual(values = pal2)
```
...
# exclude weekends here to comapre wednesday to other weekdays
...
summary(m_week_month_keyboard_t)
...
top_row_sobp =  cowplot::plot_grid(timeplt_sobp, or_plot_sobp, labels = c('A', 'B'), rel_widths = c(1.5, 1))


cowplot::save_plot(top_row_sobp, filename = '../../figures/sobp/design_features.png', base_width = 12, base_height = 4)

ggsave(or_plot_sobp, filename = '../../figures/sobp/design_features.png', height = 6, width = 9)
```
